...
def create_population_table(countries, cur, conn):
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS country_data (country_id INTEGER PRIMARY KEY, country TEXT, population INTEGER, male_life_expectancy INTEGER, female_life_expectancy INTEGER, internet_users INTEGER, gap_expectancies INTEGER)")
    ...
    for country in countries[first: first+25]:
        ...
        cur.execute("INSERT OR IGNORE INTO country_data (country_id,country,population,male_life_expectancy,female_life_expectancy,internet_users, gap_expectancies) VALUES (?,?,?,?,?,?,?)",(country_id,countries_name,population,male_life_expectancy,female_life_expectancy,internet_users,gap_expectancies))
